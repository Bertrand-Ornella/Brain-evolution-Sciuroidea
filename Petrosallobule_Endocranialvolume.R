...
#Create model PGLS regression line for each locomotion
Arbline_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=Arb_Tree,fixed=T), data=Arb)
NArbline_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=NArb_Tree,fixed=T), data=NArb)

#Prepare PGLS for each locomotor mode
pgls.fit.Arb <- predict(Arbline_Br_B) 
predframe.Arb <- with(Arb, data.frame(Species, EcoA, Brain.mg, PL = pgls.fit.Arb))

pgls.fit.NArb <- predict(NArbline_Br_B) 
predframe.NArb <- with(NArb, data.frame(Species, EcoA, Brain.mg, PL = pgls.fit.NArb))

#Make graph for arboreal vs. others
ggplot(squirrel.data, aes(Brain.mg, PL, color = EcoA)) +
...
#Create model PGLS regression line for each locomotion
Fosline_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=Fos_Tree), data=Fos)
NFosline_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=NFos_Tree), data=NFos)

#Prepare PGLS for each locomotor mode
pgls.fit.Fos <- predict(Fosline_Br_B) 
predframe.Fos <- with(Fos, data.frame(Species, EcoF, Brain.mg, PL = pgls.fit.Fos))

pgls.fit.NFos <- predict(NFosline_Br_B) 
predframe.NFos <- with(NFos, data.frame(Species, EcoF, Brain.mg, PL = pgls.fit.NFos))

#Make graph for fossorial vs. others
ggplot(squirrel.data, aes(Brain.mg, PL, color = EcoF)) +
...
#Create model PGLS regression line for each locomotion 
Gliline_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=Gli_Tree), data=Gli)
NGliline_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=NGli_Tree), data=NGli)

#Prepare PGLS for each locomotor mode
pgls.fit.Gli <- predict(Gliline_Br_B)
predframe.Gli <- with(Gli, data.frame(Species, EcoG, Brain.mg, PL = pgls.fit.Gli))

pgls.fit.NGli <- predict(NGliline_Br_B) 
predframe.NGli <- with(NGli, data.frame(Species, EcoG, Brain.mg, PL = pgls.fit.NGli))

#Make graph for glider vs. others
ggplot(squirrel.data, aes(Brain.mg, PL, color = EcoG)) +
...
#Create model PGLS regression line for each locomotion
Scaline_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=Sca_Tree), data=Sca)
NScaline_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=NSca_Tree), data=NSca)

#Prepare PGLS for each locomotor mode
pgls.fit.Sca <- predict(Scaline_Br_B)
predframe.Sca <- with(Sca, data.frame(Species, EcoS, Brain.mg, PL = pgls.fit.Sca))

pgls.fit.NSca <- predict(NScaline_Br_B)
predframe.NSca <- with(NSca, data.frame(Species, EcoS, Brain.mg, PL = pgls.fit.NSca))

#Make graph for scansorial vs. others
ggplot(squirrel.data, aes(Brain.mg, PL, color = EcoS)) +
...
#Create model PGLS regression line for each locomotion
Terline_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=Ter_Tree), data=Ter)
NTerline_Br_B <-gls(PL ~ Brain.mg, correlation=corPagel (1,phy=NTer_Tree), data=NTer)

#Prepare PGLS for each locomotor mode
pgls.fit.Ter <- predict(Terline_Br_B)
predframe.Ter <- with(Ter, data.frame(Species, EcoT, Brain.mg, PL = pgls.fit.Ter))

pgls.fit.NTer <- predict(NTerline_Br_B)
predframe.NTer <- with(NTer, data.frame(Species, EcoT, Brain.mg, PL = pgls.fit.NTer))

#Make graph for glider vs. others
ggplot(squirrel.data, aes(Brain.mg, PL, color = EcoT)) +

...
######### Code - Can endocranial volume and locomotion predict olfactory bulb size? ############
...
ggplot(squirrel.data, aes(Brain.mg, OB, color = Family)) +
  ...
  labs(x = "log(Endocranial volume)", y = "log(Olfactory bulb volume)") +
...
m.ols<-gls(OB ~ Brain.mg + Locomotion, data=squirrel.data, method="ML")
m.ols.body<-gls(OB ~ Brain.mg + Locomotion*Brain.mg, data=squirrel.data, method="ML")
m.ols.loco<-gls(OB ~ Brain.mg*Locomotion, data=squirrel.data, method="ML") #same as above

#Chosing the model to use
anova(m.ols, m.ols.loco) # m.ols with lowest AIC
summary(m.ols)
...
ggplot(squirrel.data, aes(x=Spe, y = oj, color = Family))+
  ...
  geom_point() +
  geom_abline(intercept = 0, slope = 0) ## pattern, i.e., green ones are very high so need PGLS

##########################  Analyses -- PGLS ###############################

### Select PGLS model
# Model with locomotion only
Lambda<-gls(OB ~ Brain.mg + Locomotion, data=squirrel.data, 
            correlation=corPagel(value=1,phy=tree_squirrel), method="ML")
Brownian<-gls(OB ~ Brain.mg + Locomotion, data=squirrel.data, 
              correlation=corBrownian(1,phy=tree_squirrel), method="ML")
OU<-gls(OB ~ Brain.mg + Locomotion, data=squirrel.data, 
        correlation=corMartins(1,phy=tree_squirrel,fixed = TRUE), method="ML")
Blomberg<-gls(OB ~ Brain.mg + Locomotion, data=squirrel.data, 
...
Lambda1<-gls(OB ~ Brain.mg + Locomotion*Brain.mg, data=squirrel.data, 
             correlation=corPagel(value=1,phy=tree_squirrel), method="ML")
Brownian1<-gls(OB ~ Brain.mg + Locomotion*Brain.mg, data=squirrel.data, 
               correlation=corBrownian(1,phy=tree_squirrel), method="ML")
OU1<-gls(OB ~ Brain.mg + Locomotion*Brain.mg, data=squirrel.data, 
         correlation=corMartins(1,phy=tree_squirrel, fixed = TRUE), method="ML")
Blomberg1<-gls(OB ~ Brain.mg + Locomotion*Brain.mg, data=squirrel.data, 
               correlation=corBlomberg(1.5,phy=tree_squirrel,fixed = TRUE), method="ML") #if 1, this is a Brownian model
...
anova(Lambda,Lambda1) #Final model - Lambda
summary(Lambda)

#Lambda = 0.69 (OB relative size shows some phylogenetic signal)
#Coefficients: p-value interpretation:
#Locomotion does not predict olfactory bulb size (p-value = 0.18)
#Endocranial volume alone can predict brain size (p-value = 0)
...
interaction_frame <- rrpp.data.frame(ob=OB,brain=Brain,loco=Ecology)
fit<-lm.rrpp(ob~brain+loco,SS.type = c("I"),data=interaction_frame)
summary(fit, formula = FALSE)
anova(fit) # When phylogeny is NOT taken into account, locomotion predict olfactory bulb size
...
Arbline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=Arb_Tree), data=Arb)
Fosline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=Fos_Tree), data=Fos)
Gliline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=Gli_Tree), data=Gli)
Scaline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=Sca_Tree), data=Sca)
Terline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=Ter_Tree), data=Ter)

#Prepare PGLS for each locomotor mode
pgls.fit.Arb <- predict(Arbline_Br_B) 
predframe.Arb <- with(Arb, data.frame(Species, Ecology, Brain.mg, OB = pgls.fit.Arb))

pgls.fit.Fos <- predict(Fosline_Br_B) 
predframe.Fos <- with(Fos, data.frame(Species, Ecology, Brain.mg, OB = pgls.fit.Fos))

pgls.fit.Gli <- predict(Gliline_Br_B) 
predframe.Gli <- with(Gli, data.frame(Species, Ecology, Brain.mg, OB = pgls.fit.Gli))

pgls.fit.Sca <- predict(Scaline_Br_B) 
predframe.Sca <- with(Sca, data.frame(Species, Ecology, Brain.mg, OB = pgls.fit.Sca))

pgls.fit.Ter <- predict(Terline_Br_B) 
predframe.Ter <- with(Ter, data.frame(Species, Ecology, Brain.mg, OB = pgls.fit.Ter))

#Make graph with PGLS corrected regressions
ggplot(squirrel.data, aes(Brain.mg, OB, color = Ecology)) +
...
#Create model PGLS regression line for each locomotion
Arbline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=Arb_Tree), data=Arb)
NArbline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=NArb_Tree), data=NArb)

#Prepare PGLS for each locomotor mode
pgls.fit.Arb <- predict(Arbline_Br_B)
predframe.Arb <- with(Arb, data.frame(Species, EcoA, Brain.mg, OB = pgls.fit.Arb))

pgls.fit.NArb <- predict(NArbline_Br_B)
predframe.NArb <- with(NArb, data.frame(Species, EcoA, Brain.mg, OB = pgls.fit.NArb))

#Make graph for arboreal vs. others
ggplot(squirrel.data, aes(Brain.mg, OB, color = EcoA)) +
...
#Create model PGLS regression line for each locomotion
Fosline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=Fos_Tree), data=Fos)
NFosline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=NFos_Tree), data=NFos)

#Prepare PGLS for each locomotor mode
pgls.fit.Fos <- predict(Fosline_Br_B)
predframe.Fos <- with(Fos, data.frame(Species, EcoF, Brain.mg, OB = pgls.fit.Fos))

pgls.fit.NFos <- predict(NFosline_Br_B)
predframe.NFos <- with(NFos, data.frame(Species, EcoF, Brain.mg, OB = pgls.fit.NFos))

#Make graph for fossorial vs. others
ggplot(squirrel.data, aes(Brain.mg, OB, color = EcoF)) +
...
#Create model PGLS regression line for each locomotion
Gliline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=Gli_Tree), data=Gli)
NGliline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=NGli_Tree), data=NGli)

#Prepare PGLS for each locomotor mode
pgls.fit.Gli <- predict(Gliline_Br_B)
predframe.Gli <- with(Gli, data.frame(Species, EcoG, Brain.mg, OB = pgls.fit.Gli))

pgls.fit.NGli <- predict(NGliline_Br_B)
predframe.NGli <- with(NGli, data.frame(Species, EcoG, Brain.mg, OB = pgls.fit.NGli))

#Make graph for glider vs. others
ggplot(squirrel.data, aes(Brain.mg, OB, color = EcoG)) +
...
#Create model PGLS regression line for each locomotion
Scaline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=Sca_Tree), data=Sca)
NScaline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=NSca_Tree), data=NSca)

#Prepare PGLS for each locomotor mode
pgls.fit.Sca <- predict(Scaline_Br_B)
predframe.Sca <- with(Sca, data.frame(Species, EcoS, Brain.mg, OB = pgls.fit.Sca))

pgls.fit.NSca <- predict(NScaline_Br_B)
predframe.NSca <- with(NSca, data.frame(Species, EcoS, Brain.mg, OB = pgls.fit.NSca))

#Make graph for scansorial vs. others
ggplot(squirrel.data, aes(Brain.mg, OB, color = EcoS)) +
...
#Create model PGLS regression line for each locomotion 
Terline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=Ter_Tree), data=Ter)
NTerline_Br_B <-gls(OB ~ Brain.mg, correlation=corPagel (1,phy=NTer_Tree), data=NTer)

#Prepare PGLS for each locomotor mode
pgls.fit.Ter <- predict(Terline_Br_B) 
predframe.Ter <- with(Ter, data.frame(Species, EcoT, Brain.mg, OB = pgls.fit.Ter))

pgls.fit.NTer <- predict(NTerline_Br_B) 
predframe.NTer <- with(NTer, data.frame(Species, EcoT, Brain.mg, OB = pgls.fit.NTer))

#Make graph for terrestrial vs. others
ggplot(squirrel.data, aes(Brain.mg, OB, color = EcoT)) +

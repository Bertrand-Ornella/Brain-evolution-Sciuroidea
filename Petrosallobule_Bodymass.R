...
######### Code - Can body mass and locomotion predict petrosal lobule size? ############
...
ggplot(squirrel.data, aes(Body.mg, PL, color = Family)) +
...
m.ols<-gls(PL ~ Body.mg + Locomotion, data=squirrel.data, method="ML")
m.ols.body<-gls(PL ~ Body.mg + Locomotion*Body.mg, data=squirrel.data, method="ML")
m.ols.loco<-gls(PL ~ Body.mg*Locomotion, data=squirrel.data, method="ML") #same as above
...
Lambda<-gls(PL ~ Body.mg + Locomotion, data=squirrel.data, 
            correlation=corPagel(value=1,phy=tree_squirrel), method="ML")
Brownian<-gls(PL ~ Body.mg + Locomotion, data=squirrel.data, 
              correlation=corBrownian(1,phy=tree_squirrel), method="ML")
OU<-gls(PL ~ Body.mg + Locomotion, data=squirrel.data, 
        correlation=corMartins(1,phy=tree_squirrel), method="ML")
Blomberg<-gls(PL ~ Body.mg + Locomotion, data=squirrel.data, 
...
Lambda1<-gls(PL ~ Body.mg + Locomotion*Body.mg, data=squirrel.data, 
             correlation=corPagel(value=1,phy=tree_squirrel), method="ML")
Brownian1<-gls(PL ~ Body.mg + Locomotion*Body.mg, data=squirrel.data, 
               correlation=corBrownian(1,phy=tree_squirrel), method="ML")
OU1<-gls(PL ~ Body.mg + Locomotion*Body.mg, data=squirrel.data, 
         correlation=corMartins(1,phy=tree_squirrel, fixed = TRUE), method="ML")
Blomberg1<-gls(PL ~ Body.mg + Locomotion*Body.mg, data=squirrel.data, 
               correlation=corBlomberg(1.5,phy=tree_squirrel,fixed = TRUE), method="ML") #if 1, this is a Brownian model
...
#Lambda = 0.54 (brain size shows some phylogenetic signal)
#Coefficients: p-value interpretation:
#Locomotion alone does not predict brain size (p-value = 0.056)
#Body mass alone and the interaction between Body mass and locomotion 
#can predict brain size (p-value = 0 and 0.027)
...
Body<-log10(squirrel.data$Body_mass_mg)
Ecology<-squirrel.data$Ecology

#Pairwise comparisons - non-phylogenetic! (Weisbecker et al., 2019; line 552 in Analyses)
interaction_frame <- rrpp.data.frame(pl=PL,body=Body,loco=Ecology)
fit<-lm.rrpp(pl~body+loco*body,SS.type = c("I"),data=interaction_frame)
...
Gli_species<-c("Pteromyscus_pulverulentus","Aeromys_tephromelas","Pteromys_volans",
               "Petaurista_petaurista","Hylopetes_spadiceus","Glaucomys_volans")
...
Arbline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=Arb_Tree), data=Arb)
Fosline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=Fos_Tree), data=Fos)
Gliline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=Gli_Tree), data=Gli)
Scaline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=Sca_Tree), data=Sca)
Terline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=Ter_Tree), data=Ter)

#Prepare PGLS for each locomotor mode
pgls.fit.Arb <- predict(Arbline_Br_B)
predframe.Arb <- with(Arb, data.frame(Species, Ecology, Body.mg, PL = pgls.fit.Arb))

pgls.fit.Fos <- predict(Fosline_Br_B) 
predframe.Fos <- with(Fos, data.frame(Species, Ecology, Body.mg, PL = pgls.fit.Fos))

pgls.fit.Gli <- predict(Gliline_Br_B) 
predframe.Gli <- with(Gli, data.frame(Species, Ecology, Body.mg, PL = pgls.fit.Gli))

pgls.fit.Sca <- predict(Scaline_Br_B) 
predframe.Sca <- with(Sca, data.frame(Species, Ecology, Body.mg, PL = pgls.fit.Sca))

pgls.fit.Ter <- predict(Terline_Br_B) 
predframe.Ter <- with(Ter, data.frame(Species, Ecology, Body.mg, PL = pgls.fit.Ter))

#Make graph with PGLS corrected regressions
ggplot(squirrel.data, aes(Body.mg, PL, color = Ecology)) +
...
#Create model PGLS regression line for each locomotion
Arbline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=Arb_Tree), data=Arb)
NArbline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=NArb_Tree), data=NArb)

#Prepare PGLS for each locomotor mode
pgls.fit.Arb <- predict(Arbline_Br_B)
predframe.Arb <- with(Arb, data.frame(Species, EcoA, Body.mg, PL = pgls.fit.Arb))

pgls.fit.NArb <- predict(NArbline_Br_B)
predframe.NArb <- with(NArb, data.frame(Species, EcoA, Body.mg, PL = pgls.fit.NArb))

#Make graph for arboreal vs. others
ggplot(squirrel.data, aes(Body.mg, PL, color = EcoA)) +
...
#Create model PGLS regression line for each locomotion
Fosline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=Fos_Tree), data=Fos)
NFosline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=NFos_Tree), data=NFos)

#Prepare PGLS for each locomotor mode
pgls.fit.Fos <- predict(Fosline_Br_B)
predframe.Fos <- with(Fos, data.frame(Species, EcoF, Body.mg, PL = pgls.fit.Fos))

pgls.fit.NFos <- predict(NFosline_Br_B)
predframe.NFos <- with(NFos, data.frame(Species, EcoF, Body.mg, PL = pgls.fit.NFos))

#Make graph for fossorial vs. others
ggplot(squirrel.data, aes(Body.mg, PL, color = EcoF)) +
...
#Create model PGLS regression line for each locomotion
Gliline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=Gli_Tree), data=Gli)
NGliline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=NGli_Tree), data=NGli)

#Prepare PGLS for each locomotor mode
pgls.fit.Gli <- predict(Gliline_Br_B)
predframe.Gli <- with(Gli, data.frame(Species, EcoG, Body.mg, PL = pgls.fit.Gli))

pgls.fit.NGli <- predict(NGliline_Br_B)
predframe.NGli <- with(NGli, data.frame(Species, EcoG, Body.mg, PL = pgls.fit.NGli))

#Make graph for glider vs. others
ggplot(squirrel.data, aes(Body.mg, PL, color = EcoG)) +
...
#Create model PGLS regression line for each locomotion
Scaline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=Sca_Tree), data=Sca)
NScaline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=NSca_Tree), data=NSca)

#Prepare PGLS for each locomotor mode
pgls.fit.Sca <- predict(Scaline_Br_B)
predframe.Sca <- with(Sca, data.frame(Species, EcoS, Body.mg, PL = pgls.fit.Sca))

pgls.fit.NSca <- predict(NScaline_Br_B)
predframe.NSca <- with(NSca, data.frame(Species, EcoS, Body.mg, PL = pgls.fit.NSca))

#Make graph for scansorial vs. others
ggplot(squirrel.data, aes(Body.mg, PL, color = EcoS)) +
...
#Create model PGLS regression line for each locomotion
Terline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=Ter_Tree), data=Ter)
NTerline_Br_B <-gls(PL ~ Body.mg, correlation=corPagel (1,phy=NTer_Tree), data=NTer)

#Prepare PGLS for each locomotor mode
pgls.fit.Ter <- predict(Terline_Br_B)
predframe.Ter <- with(Ter, data.frame(Species, EcoT, Body.mg, PL = pgls.fit.Ter))

pgls.fit.NTer <- predict(NTerline_Br_B)
predframe.NTer <- with(NTer, data.frame(Species, EcoT, Body.mg, PL = pgls.fit.NTer))

#Make graph for terrestrial vs. others
ggplot(squirrel.data, aes(Body.mg, PL, color = EcoT)) +
...
#Import squirrel data - to run the following
squirrel.data1<-read.csv("squirrels_PEQ_res.csv", header=T, row.names = 1)
squirrel.data<-squirrel.data1[-c(23),]

#Select data
data<-squirrel.data[,8:15]
Y<-"Petrosal_lobule_volume_mm3"
X<-"Body_mass_mg"

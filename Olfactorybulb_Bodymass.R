...
######### Code - Can body mass and locomotion predict olfactory bulb size? ############
...
squirrel.data$Olfactory_bulb_volume_mm3<-log10(squirrel.data$Olfactory_bulb_volume_mm3)
names(squirrel.data)[names(squirrel.data) == "Olfactory_bulb_volume_mm3"] <- "OB"

squirrel.data$Body_mass_mg<-log10(squirrel.data$Body_mass_mg)
names(squirrel.data)[names(squirrel.data) == "Body_mass_mg"] <- "Body.mg"
...
ggplot(squirrel.data, aes(Body.mg, OB, color = Family)) +
  ...
  labs(x ="log(Body mass)", y = "log(Olfactory bulb volume)") +
...
m.ols<-gls(OB ~ Body.mg + Locomotion, data=squirrel.data, method="ML")
m.ols.body<-gls(OB ~ Body.mg + Locomotion*Body.mg, data=squirrel.data, method="ML")
m.ols.loco<-gls(OB ~ Body.mg*Locomotion, data=squirrel.data, method="ML") #same as above

#Chosing the model to use
anova(m.ols, m.ols.body, m.ols.loco)
summary(m.ols)
...
Lambda<-gls(OB ~ Body.mg + Locomotion, data=squirrel.data, 
            correlation=corPagel(value=0.3,phy=tree_squirrel), method="ML") #if value >0.3, need fixed value for lambda and at 1 = brownian
Brownian<-gls(OB ~ Body.mg + Locomotion, data=squirrel.data, 
              correlation=corBrownian(1,phy=tree_squirrel), method="ML")
OU<-gls(OB ~ Body.mg + Locomotion, data=squirrel.data, 
        correlation=corMartins(1,phy=tree_squirrel), method="ML")
Blomberg<-gls(OB ~ Body.mg + Locomotion, data=squirrel.data, 
...
Lambda1<-gls(OB ~ Body.mg + Locomotion*Body.mg, data=squirrel.data, 
             correlation=corPagel(value=0.3,phy=tree_squirrel), method="ML") #if value >0.3, need fixed value for lambda and at 1 = brownian
Brownian1<-gls(OB ~ Body.mg + Locomotion*Body.mg, data=squirrel.data, 
               correlation=corBrownian(1,phy=tree_squirrel), method="ML")
OU1<-gls(OB ~ Body.mg + Locomotion*Body.mg, data=squirrel.data, 
         correlation=corMartins(1,phy=tree_squirrel), method="ML")
Blomberg1<-gls(OB ~ Body.mg + Locomotion*Body.mg, data=squirrel.data, 
               correlation=corBlomberg(1.5,phy=tree_squirrel,fixed = TRUE), method="ML") #if 1, this is a Brownian model

#Chosing the best model using AIC
anova(Lambda,Brownian,OU, Blomberg) # OU is the best but not significantly different from Lambda
anova(Lambda1,Brownian1,OU1, Blomberg1) # OU1 is the best but not significantly different from Lambda1
anova(OU,OU1) # not significantly different but OU chosen because less complex
summary(OU) #Final model

#Coefficients: p-value interpretation:
#Body mass alone predicts but Locomotion cannot predict olfactory bulb size (p-value > 0.05)

#To obtain lambda
summary(Lambda)
#Lambda = 0.29 (olfactory bulb size shows little phylogenetic signal)
...
squirrel.data<-read.csv("squirrels_PEQ_res.csv", header=T)
OB<-log10(squirrel.data$Olfactory_bulb_volume_mm3)
...
interaction_frame <- rrpp.data.frame(ob=OB,body=Body,loco=Ecology)
fit<-lm.rrpp(ob~body+loco,SS.type = c("I"),data=interaction_frame)
summary(fit, formula = FALSE)
anova(fit) # When phylogeny is NOT taken into account, locomotion does not predict olfactory bulb size

#No pairwise performed because p-value <0.05 for locomotion 
...
Arbline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=Arb_Tree), data=Arb)
Fosline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=Fos_Tree), data=Fos)
Gliline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=Gli_Tree), data=Gli)
Scaline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=Sca_Tree), data=Sca)
Terline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=Ter_Tree), data=Ter)

#Prepare PGLS for each locomotor mode
pgls.fit.Arb <- predict(Arbline_Br_B)
predframe.Arb <- with(Arb, data.frame(Species, Ecology, Body.mg, OB = pgls.fit.Arb))

pgls.fit.Fos <- predict(Fosline_Br_B)
predframe.Fos <- with(Fos, data.frame(Species, Ecology, Body.mg, OB = pgls.fit.Fos))

pgls.fit.Gli <- predict(Gliline_Br_B)
predframe.Gli <- with(Gli, data.frame(Species, Ecology, Body.mg, OB = pgls.fit.Gli))

pgls.fit.Sca <- predict(Scaline_Br_B)
predframe.Sca <- with(Sca, data.frame(Species, Ecology, Body.mg, OB = pgls.fit.Sca))

pgls.fit.Ter <- predict(Terline_Br_B)
predframe.Ter <- with(Ter, data.frame(Species, Ecology, Body.mg, OB = pgls.fit.Ter))

#Make graph with PGLS corrected regressions
ggplot(squirrel.data, aes(Body.mg, OB, color = Ecology)) +
...
#Create model PGLS regression line for each locomotion
Arbline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=Arb_Tree), data=Arb)
NArbline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=NArb_Tree), data=NArb)

#Prepare PGLS for each locomotor mode
pgls.fit.Arb <- predict(Arbline_Br_B)
predframe.Arb <- with(Arb, data.frame(Species, EcoA, Body.mg, OB = pgls.fit.Arb))

pgls.fit.NArb <- predict(NArbline_Br_B)
predframe.NArb <- with(NArb, data.frame(Species, EcoA, Body.mg, OB = pgls.fit.NArb))

#Make graph for arboreal vs. others
ggplot(squirrel.data, aes(Body.mg, OB, color = EcoA)) +
...
#Create model PGLS regression line for each locomotion
Fosline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=Fos_Tree), data=Fos)
NFosline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=NFos_Tree), data=NFos)

#Prepare PGLS for each locomotor mode
pgls.fit.Fos <- predict(Fosline_Br_B)
predframe.Fos <- with(Fos, data.frame(Species, EcoF, Body.mg, OB = pgls.fit.Fos))

pgls.fit.NFos <- predict(NFosline_Br_B)
predframe.NFos <- with(NFos, data.frame(Species, EcoF, Body.mg, OB = pgls.fit.NFos))

#Make graph for fossorial vs. others
ggplot(squirrel.data, aes(Body.mg, OB, color = EcoF)) +
...
#Create model PGLS regression line for each locomotion
Gliline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=Gli_Tree), data=Gli)
NGliline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=NGli_Tree), data=NGli)

#Prepare PGLS for each locomotor mode
pgls.fit.Gli <- predict(Gliline_Br_B)
predframe.Gli <- with(Gli, data.frame(Species, EcoG, Body.mg, OB = pgls.fit.Gli))

pgls.fit.NGli <- predict(NGliline_Br_B)
predframe.NGli <- with(NGli, data.frame(Species, EcoG, Body.mg, OB = pgls.fit.NGli))

#Make graph for glider vs. others
ggplot(squirrel.data, aes(Body.mg, OB, color = EcoG)) +
...
#Create model PGLS regression line for each locomotion
Scaline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=Sca_Tree), data=Sca)
NScaline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=NSca_Tree), data=NSca)

#Prepare PGLS for each locomotor mode
pgls.fit.Sca <- predict(Scaline_Br_B)
predframe.Sca <- with(Sca, data.frame(Species, EcoS, Body.mg, OB = pgls.fit.Sca))

pgls.fit.NSca <- predict(NScaline_Br_B)
predframe.NSca <- with(NSca, data.frame(Species, EcoS, Body.mg, OB = pgls.fit.NSca))

#Make graph for scansorial vs. others
ggplot(squirrel.data, aes(Body.mg, OB, color = EcoS)) +
...
#Create model PGLS regression line for each locomotion
Terline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=Ter_Tree), data=Ter)
NTerline_Br_B <-gls(OB ~ Body.mg, correlation=corMartins (1,phy=NTer_Tree), data=NTer)

#Prepare PGLS for each locomotor mode
pgls.fit.Ter <- predict(Terline_Br_B)
predframe.Ter <- with(Ter, data.frame(Species, EcoT, Body.mg, OB = pgls.fit.Ter))

pgls.fit.NTer <- predict(NTerline_Br_B)
predframe.NTer <- with(NTer, data.frame(Species, EcoT, Body.mg, OB = pgls.fit.NTer))

#Make graph for terrestrial vs. others
ggplot(squirrel.data, aes(Body.mg, OB, color = EcoT)) +
